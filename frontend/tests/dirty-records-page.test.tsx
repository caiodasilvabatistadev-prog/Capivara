...
test("lista e filtra pessoas vivas e falecidas", async () => {
  ...
  expect(screen.getAllByText(/Condenad[oa]/)).toHaveLength(6);
  ...
  await user.click(screen.getByRole("button", { name: "Pessoas falecidas" }));
  expect(screen.getByRole("heading", { name: "José Maria Marin" })).toBeVisible();
  expect(screen.queryByRole("heading", { name: "Jair Bolsonaro" })).not.toBeInTheDocument();
});
