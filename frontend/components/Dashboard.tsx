...
export default function Dashboard({ data, onBack }: { data: DashboardData; onBack: () => void }) {
  ...
  function openLegislativeMetric(label: string) {
    if (label === "Propostas de sua autoria" || label === "Propostas relatadas") {
      window.dispatchEvent(new CustomEvent("open-proposals", { detail: label === "Propostas de sua autoria" ? "authored" : "reported" }));
      document.getElementById("propostas-detalhadas")?.scrollIntoView({ behavior: "smooth", block: "start" });
    } else if (label === "Votações nominais em Plenário") {
      document.getElementById("perfil-votacoes")?.scrollIntoView({ behavior: "smooth", block: "start" });
    }
  }
  function renderGroup(group: string) {
    return <div key={group}><section aria-label={group} className="mb-10"><h3 className="mb-4 text-xl font-bold">{group}</h3><div className={`grid gap-4 sm:grid-cols-2 ${group === "Gastos públicos" ? "" : "lg:grid-cols-3"}`}>{data.metrics.filter(metric => metric.group === group).map(metric => { const label = labels[metric.label] || metric.label; const actionable = ["Propostas de sua autoria", "Propostas relatadas", "Votações nominais em Plenário"].includes(label); const content = <><h4 className="mb-3 text-sm font-semibold text-slate-600"><GlossaryTerm term={label}>{label}</GlossaryTerm></h4><p className={"break-words font-bold tracking-tight text-emerald-950 " + (metric.value.length > 80 ? "text-base leading-relaxed" : "text-2xl")}>{metric.value.replace(/^1 dias$/, "1 dia").replace(/^1 reuniões$/, "1 reunião")}</p><p className="mt-3 text-xs leading-relaxed text-slate-500">{metric.explanation}</p>{actionable && <p className="mt-4 text-sm font-bold text-emerald-800">Ver o que foi feito →</p>}</>; return actionable ? <button className="rounded-2xl border bg-white p-5 text-left transition hover:border-emerald-700 hover:shadow-md" key={metric.label} onClick={() => openLegislativeMetric(label)}>{content}</button> : <article className="rounded-2xl border bg-white p-5" key={metric.label}>{content}</article>; })}</div></section>{group === "Atividade legislativa" && <Proposals person={person} initialYear={data.year} onLoaded={section => setParliament(old => [...old.filter(item => item.title !== section.title), section])} />}</div>;
  }
