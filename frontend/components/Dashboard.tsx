...
export default function Dashboard({ data, onBack }: { data: DashboardData; onBack: () => void }) {
  ...
  const groups = ["Gastos públicos", "Presença", "Atividade legislativa", "Recursos e benefícios"];
  ...
  return <section aria-label="Perfil" className="mt-8">
    ...
    <header className="rounded-2xl border bg-white p-7 sm:flex sm:items-center sm:gap-6">
      ...
      <div><p className="mb-2 text-xs font-semibold uppercase tracking-widest text-slate-500">Deputado federal · {person.state}</p><h2 className="text-3xl font-bold">{person.name}</h2><p className="mt-2 text-slate-600">{person.party} · {person.email || "E-mail não informado"}</p><p className="mt-3 text-sm font-medium text-emerald-800">Dados oficiais consultados na Câmara dos Deputados.</p></div>
    </header>
    ...
    <h3 className="mb-4 text-xl font-bold">Todos os dados da página oficial</h3><p className="mb-5 text-sm text-slate-600">Abra uma seção para ler contatos, emendas, comissões, cargos, tabelas, agenda e notícias sem sair daqui. Percentuais das tabelas são reproduzidos como publicados; eventuais inconsistências pertencem à fonte.</p>
