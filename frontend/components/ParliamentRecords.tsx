...
function Panel({ person, kind, onLoaded, onClear }: { person: Politician; kind: "amendments" | "votes"; onLoaded: (data: Section) => void; onClear?: (title: string) => void }) {
  ...
  const rows = (table?.rows.slice(1) || []).filter(row => {
    ...
    return text.includes(folded(query)) && (topic === "Todos" || (keywords[topic] || []).some(key => text.includes(key)));
  });
  return <section aria-label={title} className="mb-10 rounded-2xl border bg-white p-6">
    ...
    <p className="my-4 text-sm leading-relaxed text-slate-600">{kind === "amendments" ? "Veja as Transferências Especiais, conhecidas como Emendas Pix, com ente beneficiário, finalidade, valores previstos e empenhados. Para deputados, a consulta também reúne as demais emendas publicadas pela Câmara." : "O assunto aparece primeiro em linguagem simples, seguido do voto e do texto original. Sim ou Não se refere ao objeto desta votação; sem registro não significa falta."}</p>
