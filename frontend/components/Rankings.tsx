...
function RankingChart({ data, provider, onOpen }: { data: Ranking; provider: string; onOpen: (provider: string, id: number) => void }) {
  const maximum = Math.max(...data.entries.map(item => Number(item.value) || 0), 1);
  return <div className="mt-5" aria-label={`Gráfico de barras: ${data.title}`}>
    {data.entries.length > 0 && <div className="mb-3 flex items-center justify-between gap-3 text-[11px] text-slate-500"><span>Quanto maior a faixa, maior o valor</span><span>Maior resultado = 100%</span></div>}
    <ol className="space-y-3">{data.entries.map(item => {
      const width = Math.max(3, (Number(item.value) || 0) / maximum * 100);
      return <li className={"relative min-h-28 overflow-hidden rounded-xl border bg-slate-50 p-4 " + (item.party ? "party-ranking" : "")} key={item.id || item.name}>
        <span aria-hidden="true" className="absolute inset-y-0 left-0 transition-[width] duration-500" style={{ width: `${width}%`, background: chartColors[data.metric] || "rgba(16, 185, 129, .14)" }} />
