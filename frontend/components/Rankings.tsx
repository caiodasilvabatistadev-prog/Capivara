...
export type Ranking = {
  ...
  entries: { position: number; id: number | null; name: string; value: string; detail: string }[];
};
...
function Card({ provider, metric, year, title, onOpen }: { provider: string; metric: string; year: number; title: string; onOpen: (provider: string, id: number) => void }) {
  ...
  return <article className="min-w-0 rounded-2xl border bg-white p-5">
    ...
    {data && <div className="mt-5">
      ...
      <ol className="space-y-3">{data.entries.map(item => <li className="rounded-xl border bg-slate-50 p-4" key={item.id || item.name}>
        <div className="flex items-start gap-3"><span className="text-sm font-bold text-emerald-800">{item.position}º</span><div className="min-w-0 flex-1"><p className="break-words font-semibold">{item.name}</p><p className="mt-1 text-lg font-bold text-emerald-950">{data.unit === "BRL" ? money.format(Number(item.value)) : Number(item.value).toLocaleString("pt-BR") + " " + data.unit}</p>{item.detail && <p className="mt-2 text-xs leading-relaxed text-slate-500">{item.detail}</p>}
        {item.id !== null && <button onClick={() => onOpen(provider, item.id!)} className="mt-3 text-xs font-bold text-emerald-800">Ver dashboard de {item.name}</button>}</div></div>
      </li>)}</ol>
...
export default function Rankings({ provider, onOpen }: { provider: string; onOpen: (provider: string, id: number) => void }) {
  ...
  return <section aria-label="Rankings de dados públicos" className="mt-8">
    ...
    <p className="mb-5 text-sm leading-relaxed text-slate-500">A seleção do poder e da casa na busca também organiza estas comparações. Gastos, projetos e faltas são indicadores distintos; não formam uma nota política. Os dados são consultados quando você abre cada lista.</p>
    <div className="grid items-start gap-4 lg:grid-cols-3">{[["expenses", "Maiores gastos com cota parlamentar"], ["approved", "Mais projetos aprovados"], ["absences", "Maiores ausências em Plenário"]].map(([metric, title]) => <Card key={provider + metric + year} provider={provider} metric={metric} year={year} title={title} onOpen={onOpen} />)}</div>
